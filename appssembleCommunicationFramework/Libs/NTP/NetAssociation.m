...
uint32_t ntp2unix(union ntpTime time) {
    uint32_t seconds = 0;
    seconds = time.partials.wholeSeconds - JAN_1970;
...
//    NSDate *date = [NSDate dateWithTimeIntervalSince1970:ntp2unix(ntpClientSendTime)];
    
    return ntpClientSendTime;
}
...
//      NTP_Logging(@"%@", [self prettyPrintTimers]);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{ [_delegate netAssociationHasFinishSync:self]; });// tell delegate we're done
}

- (void) netAssociationHasFinishSync:(NetAssociation *)netAssociation {
/*┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
